...
public class SQLiteDBManager {
  ...
  private static void recordMove(Connection conn, int moveNumber, String player, int row, int col) {
    try (PreparedStatement pstmt = conn.prepareStatement(
        "INSERT INTO moves (move_number, player, row, col) VALUES (?, ?, ?, ?)")) {
  ...
  public static void addGame(List<GameMove> moves, int totalMoves, int playerMoves,
      int computerMoves, String result, int duration, String level) {
    createTables();

    try (Connection conn = getConnection();
        PreparedStatement pstmt = conn.prepareStatement(
            "INSERT INTO games (total_moves, player_moves, computer_moves, result, duration, level, game_state) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
    ...
    try (Connection conn = getConnection();
      ...
      if (tableResultSet.next()) { // Если таблица существует
        ...
        LOGGER.log(Level.WARNING,
            "Table 'games' does not exist in the database."); // Логируем предупреждение
      }
    } catch (SQLException e) {
