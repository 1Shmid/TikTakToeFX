...
import tiktaktoefx20.database.GameMoveRecorder;
...
public class GameEngine {
	...
	public void magicOn(ClickResult clickResult, GameParams params) {
		...
		// Записываем ход игрока в БД
		GameMoveRecorder.recordMove(moveCounter, "player", clickResult.row(), clickResult.col());
	...
	private void performComputerMove(GameParams params) {
		...
		GameMoveRecorder.recordMove(params.getMoveCounter(), "computer", computerMove[0],
				computerMove[1]);
	}
	...
	private List<GameMoveRecorder> convertMovesToGameMovesList() {
		List<GameMoveRecorder> gameMovesList = new ArrayList<>();
		for (Object[] move : GameMoveRecorder.getMoves()) {
			int moveNumber = (int) move[0];
			String player = (String) move[1];
			int row = (int) move[2];
			int col = (int) move[3];
			gameMovesList.add(new GameMoveRecorder(moveNumber, player, row, col));
		}
	...
	private static List<int[]> getRowsWinningCellsCoordinates(int row) {
	...
	private static List<int[]> getColumnWinningCellsCoordinates(int col) {
		List<int[]> coordinates = new ArrayList<>();
	...
	private static List<int[]> getDiagonalWinningCellsCoordinates(char[][] gameField) {
